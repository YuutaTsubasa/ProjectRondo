 ...
 * fallback resolves it regardless of `scene.activeCamera`.) `characterRig.ts` sets `scene.activeCamera`
 * and then invokes this callback with that camera, so the cascaded branch holds today — but it is an
...
export function createShadows(sun: DirectionalLight, camera: Camera, shape: ShadowShape = {}): Shadows {
  ...
  if (CascadedShadowGenerator.IsSupported) {
    ...
    // takes the knight's receiving body from 31 meshes to 40, so the shipped
    // casting-and-receiving count is now 71. Task 8 was never re-run at 71, on either branch.
    //
