...
/** The visible upper hemisphere maps to stops 0.5 (horizon) through 1 (zenith). */
function skyGradientTexture(scene: Scene): DynamicTexture {
  ...
  gradient.addColorStop(0, HORIZON_HEX);
  gradient.addColorStop(0.5, HORIZON_HEX);
  gradient.addColorStop(0.7, '#96bbc9');
  gradient.addColorStop(1, '#4f8faf');
...
export function createEnvironment(scene: Scene): Environment {
  ...
  // Broad cool fill softens foliage shadows while the warm sun still defines form.
  const ambient = new HemisphericLight('ambient', new Vector3(0, 1, 0), scene);
  ambient.intensity = 0.62;
  ambient.diffuse = new Color3(0.92, 0.97, 1);
