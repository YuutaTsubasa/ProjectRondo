<script lang="ts">
  ...
  let { onReady, onFailure }: { onReady: () => void; onFailure: (error: unknown) => void } = $props();
  ...
  onMount(() => {
    ...
    levels.swap(
      async () => {
        try { return await createHubScene(engine, canvas, enterTower); }
...
{#if ready && session && !gameMode.isPlaying}
