...
came from different machines — and, as the table notes, because the only protocol the branch defines
(`__fpsAB`) is a single-build refreshRate pairing that never touches `main`. Whoever re-measures it
should know they are getting shadow-map re-render cost, not this branch's frame cost against `main`.
