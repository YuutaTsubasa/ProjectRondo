...
| 14 | Prefer async/await with signals (`ToSignal`) over blocking waits and manual `_Process` polling | Await Godot signals (`await ToSignal(...)` / `SignalAwaiter`) instead of blocking (`.Result`/`.Wait()`) or hand-rolled per-frame polling; avoid `.ContinueWith` chains, and make fire-and-forget explicit with contained errors. The pure `ProjectRondo.Domain` uses standard `Task`/`ValueTask` (no engine). |
