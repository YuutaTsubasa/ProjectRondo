...
| Path | What |
| --- | --- |
| `src/domain/` | Pure TS: `hub/character/*` (movement), `dialogue/*` (dialogue graph + DSL), `audio/` (footstep cadence + contact phases, music director), `math/`, `kernel/`. No engine imports. Vitest-covered. |
...
| `src/presentation/audio/` | AudioV2 wiring: `audioEngine` (engine + buses), `manifest` (cue → file/bus/volume), `soundBank` (loading + missing-asset policy), `hubAudio` (per-frame scene wiring). |
