 ...
 * which is also what keeps the 42 and the 40 quoted in that doc, in `knight.ts` and in `shadows.ts`
 * from going stale. Those two figures only: `knight.ts` and `shadows.ts` also quote 71, the 40 plus
 * a 31-mesh environment half (20 tree spots, 8 pillars, the pedestal, rock, bush), and nothing here
 * pins that half — add a tree spot and every 71 goes stale with this suite still green.
 */
